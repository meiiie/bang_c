...
> **Bài nộp: image `hacamy12345/neko-core:qwen3-4b-r2-cuda128-20260627` (CUDA 12.8, Blackwell sm_120, offline, ≤5B).**
...
Bảng C Vòng 2 yêu cầu một **Docker container offline, tự chứa**: đọc `/code/private_test.json`, ghi
`/code/submission.csv` (`qid,answer`) + `/code/submission_time.csv` (`qid,answer,time` — thời gian
TỪNG câu). Chấm trên **2000 câu private đa lĩnh vực** (đề kiểu THPT
...
**Ràng buộc BTC (xác nhận chính thức 2026‑06‑18):** (1) GPU chấm **RTX 5060 Ti (Blackwell sm_120, 16GB) · RAM 32GB · base CUDA ≥ 12.8**; (2) **≤5B tính theo
...
/code/private_test.json (JSON)
  ...
  → contract repair   (đảm bảo submission.csv phủ ĐÚNG mọi qid với chữ cái hợp lệ — chống 0 điểm)
  → predict.py        → /code/submission.csv + /code/submission_time.csv (đo time TỪNG câu)
...
- **Build CUDA 12.8 · wheel native SASS `sm_120` (Blackwell — đúng RTX 5060 Ti của BTC) + PTX floor
  `compute_75`** (forward‑JIT cho mọi GPU ≥ sm_75, gồm sm_120) → chạy đúng trên GPU chấm. `GGML_NATIVE=off`
  → chạy mọi CPU (wheel prebuilt SIGILL trên CPU cũ).
- **Kiểm chứng:** smoke‑test kiểu giám khảo (mount thư mục → `/code`) trên build CUDA 12.8 → `predict.py`
  đọc `/code/private_test.json`, ghi `submission.csv` + `submission_time.csv` (đáp án thật + time/câu).
  Blackwell sm_120 **không máy nào test local được**; bài build native sm_120 + PTX floor đúng CUDA 12.8
  theo yêu cầu BTC (Q&A 2026‑06‑27) — nếu eval lỗi BTC sẽ liên hệ đội.
...
## 6. Robustness — hợp đồng submission.csv bất khả xâm phạm
...
- **submission.csv ghi TRƯỚC mọi thứ có thể raise:** bước *contract‑repair* dựng lại danh sách dự đoán phủ
...
audit.** Đã smoke‑test kiểu giám khảo (mount → `/code`) → `submission.csv` + `submission_time.csv` hợp lệ.
...
- Đo thật (GPU thuê): ~3 giây/câu → **2000 câu ≈ ~100 phút**, VRAM ~5GB → **không OOM, không timeout** trên RTX 5060 Ti 16GB.
...
> Image: `hacamy12345/neko-core:qwen3-4b-r2-cuda128-20260627` (CUDA 12.8, Blackwell sm_120, offline,
> ≤5B). Mã nguồn + tái lập: `README.md`. Bản tiếng Anh: `docs/method-writeup.md`.
